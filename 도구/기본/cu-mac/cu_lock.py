...
사용법:
  python3 cu_lock.py acquire <agent_id> [--note "작업설명"] [--ttl 600]
      → 획득 성공: 종료코드 0. 다른 에이전트 사용중: 종료코드 2 (보유자/만료시간 출력) → 기다렸다 재시도.
  python3 cu_lock.py heartbeat <agent_id> [--ttl 600]   # 긴 작업 중 주기적으로(임대시간 절반쯤마다)
  python3 cu_lock.py release <agent_id>                 # 작업 끝나면 반드시
  python3 cu_lock.py status                             # 누가 CU 중인지 확인
...
def main():
    ...
    ap.add_argument("--target", default="host", help="화면 단위(기본 host). VM이면 vms.json의 이름")
    a = ap.parse_args()
