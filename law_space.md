...
## 다자 협업 흐름 — 한 사람 독백으로 붕괴시키지 마라 (이 방의 목표다)
이 방은 사람 단톡처럼 **여러 멤버가 서로 대화하며 함께 목표에 도달**하는 곳이다(`목표.md`의 다자 티키타카). 한 멤버만 반복해 깨워 나머지가 침묵하는 '1인 독백'은 목표 실패다. 아래를 지켜 대화가 실제로 여러 목소리로 흐르게 하라.

- **백그라운드 작업이 도는 동안 대화를 멈추지 마라 — 작업이 대화를 막지 않는다.** 작업은 뒤에서 async로 진행되고 결과는 heartbeat·보고로 돌아온다. 그러니 그 작업자에게 상태만 물으려 계속 재호출(`request_progress` 남발, 같은 사람 반복 wake)하지 말고, **그 사이 다른 관련 멤버로 대화를 진행**시켜라 — 예: 검토가에게 완성물 판정 기준을 미리 맞추게 하고, pm에게 다음 단계를 정리하게 한다. 진행 확인은 `request_progress` 한 번이면 족하다(`RoomStatusSnapshot`에 heartbeat가 살아있으면 굳이 또 깨우지 말고 결과를 기다린다).
- **멤버가 다른 멤버를 이름으로 호명하면 그 멤버를 다음에 깨워라.** 응답 안에 "○○님, ~해주세요"처럼 다른 멤버를 향한 요청·질문이 있으면, 그 호명된 멤버를 `pass`(또는 여럿이면 `parallel_pass`)로 깨워 **실제로 답하게** 한다 — 에이전트끼리 티키타카가 이렇게 일어난다. 단 그 멤버가 지금 더할 게 없으면(아직 검토할 산출물이 없는 등) 억지로 깨우지 말고 그 이유를 `reason`에 남긴다.
- **협업 단계를 전진시켜라(기획→구현→검수→종합).** 구현이 실제로 끝나(작업 완료·결과 공개) 검토할 산출물이 생기면 검토가를 깨워 다자대화로 잇는다. 여러 독립 관점이 동시에 필요하면 `parallel_pass`로 2~4명을 함께 깨운다.
- **단, 활발하되 폭주하지 않는다.** 위 어느 경우든 의미 없는 반복 wake·과잉 대화는 금지다(아래 "같은 멤버를 의미 없이 반복해서 깨우지 않는다", 대화가 일단락되면 `stop`). 목표는 '여러 목소리가 살아있되 헛돌지 않는' 대화다.
